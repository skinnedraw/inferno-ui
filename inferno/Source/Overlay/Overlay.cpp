...
void RefreshConfigs()
{
    ...
    for (const auto& entry : std::filesystem::directory_iterator("C:\\Athena\\Configs")) {
...
void Overlay::Setup()
{
    ...
    ImColor Inline = FloatToColor(InlineF);
    {
        ...
        if (std::filesystem::exists("C:\\Athena\\Theme.json"))
        {
            std::ifstream file("C:\\Athena\\Theme.json");
    ...
    {
        {
            ...
            if (duration_cast<std::chrono::seconds>(now - LastThemeSave).count() >= 3) {
                ...
                std::ofstream file("C:\\Athena\\Theme.json");
                file << config.dump();
                RefreshConfigs();
                LastThemeSave = now; 
            }
        ...
        if (getbg == Globals::RobloxWindow || getbg == hwnd)
        {
            ...
            if (draw)
            {
                ...
                if (DockButtons[0].Enabled)
                {
                    ...
                    if (currenttab == "World")
                    {
                        ...
                        if (ImGui::SliderInt("##CameraFOV", &Globals::World::CameraFOVValue, 10, 135))
                        {
                            ...
                            {
                                SDK::Memory->Write<float>(Globals::Workspace.FindFirstChildOfClass("Camera").Address + SDK::Offsets::FOV, (Globals::World::CameraFOVValue * 0.0174533f));
                            }
                        }
                        ...
                        static SDK::Cache::WayPoint SelectedWaypoint;
                        style->Colors[ImGuiCol_ChildBg] = ChildColor;
                        style->Colors[ImGuiCol_Border] = Inline;
                        for (SDK::Cache::WayPoint Waypoint : SDK::Cache::Waypoints)
                        {
                            ImGui::SetCursorPosX((ImGui::GetWindowSize().x - ImGui::CalcTextSize(Waypoint.Name.c_str()).x) / 2);
                            if (SelectedWaypoint.Name == Waypoint.Name)
                            {
                                ImGui::TextColored(AccentColor.Value, Waypoint.Name.c_str());
                            }
                            else
                            {
                                ImGui::Text(Waypoint.Name.c_str());
                            }
                            if (ImGui::IsItemHovered() && ImGui::IsMouseClicked(0))
                        ...
                        if (ImGui::Button("Create", { ImGui::GetContentRegionAvail().x, 17 }))
                        {
                            ...
                            if (!AlreadyExists)
                            {
                                SDK::Cache::WayPoint Waypoint;
                                Waypoint.Name = WaypointName;
                                Waypoint.WorldPosition = SDK::Cache::LocalPlayer.HumanoidRootPart.WorldPosition;
                                SDK::Vector2 Screen = SDK::WorldToScreen(Waypoint.WorldPosition);
                                Waypoint.ScreenPosition = { Screen.x, Screen.y };
                                SDK::Cache::Waypoints.push_back(Waypoint);
                            }
                        }
                        if (ImGui::Button("Teleport", { ImGui::GetContentRegionAvail().x, 17 }))
                        {
                            if (!SelectedWaypoint.Name.empty())
                            {
                                while (SelectedWaypoint.WorldPosition != SDK::Cache::LocalPlayer.HumanoidRootPart.WorldPosition)
                                {
                                    SDK::Cache::LocalPlayer.HumanoidRootPart.Object.Primitive().SetPosition(SelectedWaypoint.WorldPosition);
                                }
                            }
                        }
                        if (ImGui::Button("Delete", { ImGui::GetContentRegionAvail().x, 17 }))
                        {
                            for (auto it = SDK::Cache::Waypoints.begin(); it != SDK::Cache::Waypoints.end(); ++it)
                            {
                                if (it->Name == SelectedWaypoint.Name)
                                {
                                    SDK::Cache::Waypoints.erase(it);
                                    SelectedWaypoint = {};
                                    break;
                                }
                            }
                        ...
                        {
                            SDK::Vector3 Color = { Globals::World::AmbientColor[0],Globals::World::AmbientColor[1],Globals::World::AmbientColor[2] };
                            if (Globals::Lighting.Address)
                            {
                                SDK::Memory->Write<SDK::Vector3>(Globals::Lighting.Address + 0xe0, Color);
                            }
                        }
                        ...
                        {
                            SDK::Vector3 Color = { Globals::World::OutdoorAmbientColor[0],Globals::World::OutdoorAmbientColor[1],Globals::World::OutdoorAmbientColor[2] };
                            if (Globals::Lighting.Address)
                            {
                                SDK::Memory->Write<SDK::Vector3>(Globals::Lighting.Address + SDK::Offsets::OutdoorAmbient, Color);
                            }
                        }
                    ...
                    if (currenttab == "Misc")
                    {
                        ...
                        if (ImGui::Button("Create", { ImGui::GetContentRegionAvail().x, 17 }))
                        {
                            if (strlen(ConfigName) > 0) {
                                ...
                                std::string fullPath = "C:\\Athena\\Configs\\" + name + ".rocks";
                    ...
                    if (currenttab == "Players")
                    {
                        ...
                        ImGui::GetWindowDrawList()->AddRect(ImGui::GetWindowPos() + ImVec2(1, 1),
                            ...
                            for (SDK::Cache::PlayerAvatar Avatar : SDK::Cache::LoadedAvatars)
                            {
                                if (Avatar.PlayerName == SelectedPlayer.PlayerName)
                                {
                                    ImGui::GetWindowDrawList()->AddImage(
                                        (ImTextureID)Avatar.Texture,
                                        ImGui::GetWindowPos() + ImVec2(1, 1),
                ...
                if (DockButtons[2].Enabled)
                {
                    ...
                    ImGui::GetWindowDrawList()->AddImage((ImTextureID)BladeeT, pos, pos + i,
                        { 0,0 },
                        { 1,1 },
                        (!Globals::Esp::Chams ? ImColor(255, 255, 255, 255) : ImColor(Globals::Esp::Colors::Chams[0], Globals::Esp::Colors::Chams[1], Globals::Esp::Colors::Chams[2]))
                    );
            ...
            if (Crosshair)
            {
                ...
                if (Globals::Aimbot::IsThereATarget && Globals::Aimbot::SilentCrosshair && Globals::Aimbot::AimType == 2)
                {
                    ...
                    cursor_point = POINT(Globals::Aimbot::SilentMousePosition.x, Globals::Aimbot::SilentMousePosition.y);
                ...
                if (Globals::Aimbot::IsThereATarget && Globals::Aimbot::SilentCrosshair && Globals::Aimbot::AimType == 2)
                {
                    ...
                    if (Globals::Aimbot::SilentCrosshairTracer)
                    {
                        ...
                        ImGui::GetBackgroundDrawList()->AddLine(
                            { realcursor.x, realcursor.y },
                            { static_cast<float>(animX),static_cast<float>(animY) },
                            ImColor(Globals::Aimbot::SilentCrosshairTracerColor[0], Globals::Aimbot::SilentCrosshairTracerColor[1], Globals::Aimbot::SilentCrosshairTracerColor[2]),
                            2.0f
                        );
                    }
                }
