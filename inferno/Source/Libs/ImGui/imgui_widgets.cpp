...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Apply the white vertical gradient overlay
    ImVec2 gradient_min = bb.Min ;
    ImVec2 gradient_max = bb.Max;

    ImU32 white_top = ImGui::GetColorU32(ImVec4(1, 1, 1, 0.10f)); // top white @ 10% alpha
    ImU32 white_bot = ImGui::GetColorU32(ImVec4(1, 1, 1, 0.00f)); // bottom white @ 0% alpha
    if (held)
    {
         white_top = ImGui::GetColorU32(ImVec4(1, 1, 1, 0.15f)); // top white @ 10% alpha
         white_bot = ImGui::GetColorU32(ImVec4(1, 1, 1, 0.05f)); // bottom white @ 0% alpha
    }
    ImGui::GetWindowDrawList()->AddRectFilledMultiColor(
        ...
        white_top,  // top-left
        white_top,  // top-right
        white_bot,  // bottom-right
        white_bot   // bottom-left
    );
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_ScrollbarGrabActive));
    ImGui::GetWindowDrawList()->AddRect(bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), GetColorU32(ImGuiCol_ScrollbarGrabHovered));
    if (g.LogEnabled)
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (is_visible)
    {
        ...
        // Fill the checkbox background with WindowBg color
        ImGui::GetWindowDrawList()->AddRectFilled(
            check_bb.Min + ImVec2(2, 2),
            check_bb.Max - ImVec2(2, 2),
            ImGui::GetColorU32(ImGuiCol_WindowBg)
        );

        // Apply the white vertical gradient overlay
        ImVec2 gradient_min = check_bb.Min + ImVec2(2, 2);
        ImVec2 gradient_max = check_bb.Max - ImVec2(2, 2);
        ...
        else if (*v)
        {
            ...
            // Draw the top-to-bottom white gradient overlay
             gradient_min = check_bb.Min + ImVec2(2, 2);
             gradient_max = check_bb.Max - ImVec2(2, 2);

             white_top = ImGui::GetColorU32(ImVec4(1, 1, 1, 0.10f)); // top 10% white
             white_bot = ImGui::GetColorU32(ImVec4(1, 1, 1, 0.00f)); // bottom 0% white
        ...
        ImGui::GetWindowDrawList()->AddRect(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_ScrollbarGrabActive));
        ImGui::GetWindowDrawList()->AddRect(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1), GetColorU32(ImGuiCol_ScrollbarGrabHovered));
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
    {
        ...
        // Apply the white vertical gradient overlay
        ImVec2 gradient_min = bb.Min + ImVec2(2, 2);
        ImVec2 gradient_max = bb.Max - ImVec2(2, 2);
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        if (popup_open)
        {
            ...
            ImGui::GetWindowDrawList()->AddText(
                ImVec2(bb.Max.x, bb.Min.y) - style.FramePadding - ImVec2(6, -3),
        ...
        else
        {
            ...
            ImGui::GetWindowDrawList()->AddText(
                ImVec2(bb.Max.x, bb.Min.y) - style.FramePadding - ImVec2(9, -3),
                GetColorU32(ImGuiCol_Text),
                "+"
            );
        }
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
        RenderTextClipped(bb.Min + style.FramePadding + ImVec2(3,0), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    }
    //if (label_size.x > 0)
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    // FIXME: This might be moved to Begin() or at least around the same spot where
    // s and other Popups are calling FindBestWindowPosForPopupEx()?
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    ImGui::GetWindowDrawList()->AddRectFilled(
        frame_bb.Min + ImVec2(2, 2),
        frame_bb.Max - ImVec2(2, 2),
        ImGui::GetColorU32(ImGuiCol_WindowBg)
    );

    // Apply the white vertical gradient overlay
    ImVec2 gradient_min = frame_bb.Min + ImVec2(2, 2);
    ImVec2 gradient_max = frame_bb.Max - ImVec2(2, 2);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        ...
        // Draw the top-to-bottom white gradient overlay
        gradient_min = frame_bb.Min + ImVec2(2, 2);
        gradient_max = grab_bb.Max - ImVec2(2, 2);
    ...
    ImGui::GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_ScrollbarGrabActive));
    ImGui::GetWindowDrawList()->AddRect(frame_bb.Min + ImVec2(1, 1), frame_bb.Max - ImVec2(1, 1), GetColorU32(ImGuiCol_ScrollbarGrabHovered));
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];

    if (data_type == ImGuiDataType_S32)
    {
        int value = *(const int*)p_data;
        int max_val = *(const int*)p_max;
        snprintf(value_buf, IM_ARRAYSIZE(value_buf), "%d/%d", value, max_val);
    }
    else if (data_type == ImGuiDataType_Float)
    {
        float value = *(const float*)p_data;
        float max_val = *(const float*)p_max;
        snprintf(value_buf, IM_ARRAYSIZE(value_buf), format ? "%.*f/%.*f" : "%.3f/%.3f",
            format ? ImParseFormatPrecision(format, 3) : 3, value,
            format ? ImParseFormatPrecision(format, 3) : 3, max_val);
    }
    else
    {
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(),17); // Arbitrary default of 8 lines high for multi-line
    ...
    if (!is_multiline)
    {
        ...
        // Apply the white vertical gradient overlay
        ImVec2 gradient_min = frame_bb.Min;
        ImVec2 gradient_max = frame_bb.Max;
    ...
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + style.FramePadding + ImVec2(0,1);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (false)
        {
            ...
            for (const char* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    // Adjust rect_pos to match the current text position to ensure proper overlap
                    rect_pos.x = draw_pos.x; // Use the same x position as the text drawing position

                    // Create the selection rectangle
                    ImRect rect(rect_pos + ImVec2(0.0f, bg_offy_up - g.FontSize),
                        rect_pos + ImVec2(rect_size.x, bg_offy_dn));

                    // Clip the rectangle against the clip rect
                    rect.ClipWith(clip_rect);

                    // Draw the selection rectangle if it's visible
        ...
        {
            // Calculate the text size for proper centering
            ImVec2 text_size = ImGui::CalcTextSize(buf_display, buf_display_end);

            // Calculate the centered x position within frame_bb
            draw_pos.x = frame_bb.Min.x + (frame_bb.Max.x - frame_bb.Min.x - text_size.x) * 0.5f;

            // Get the appropriate color based on hint state
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
           ...
           // if (flags & ImGuiInputTextFlags_ElideLeft)
            ...
            // Get appropriate color based on whether we're showing hint text
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (BeginPopup("picker"))
        {
            ...
            ImGui::GetWindowDrawList()->AddLine(ImGui::GetWindowPos() + ImVec2(2, 2),
                ...
                ImColor(ImGui::GetStyleColorVec4(ImGuiCol_CheckMark))
            );
            ImGui::GetWindowDrawList()->AddLine(ImGui::GetWindowPos() + ImVec2(2, 3),
                ImGui::GetWindowPos() + ImVec2(ImGui::GetWindowSize().x - 2, 3),
                ImColor(ImGui::GetStyleColorVec4(ImGuiCol_CheckMark).x, ImGui::GetStyleColorVec4(ImGuiCol_CheckMark).y, ImGui::GetStyleColorVec4(ImGuiCol_CheckMark).z, 0.5f)
            );
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    {
       // window->DrawList->AddRect(bb_inner.Min, bb_inner.Max, GetColorU32(ImGuiCol_ScrollbarGrabActive), rounding);
       // window->DrawList->AddRect(bb_inner.Min + ImVec2(1, 1), bb_inner.Max - ImVec2(1, 1), GetColorU32(ImGuiCol_ScrollbarGrabHovered), rounding);
        // Because GetColorU32() multiplies by the global style Alpha and we don't want to display a checkerboard if the source code had no alpha
        ImVec4 col_source = (flags & ImGuiColorEditFlags_AlphaOpaque) ? col_rgb_without_alpha : col_rgb;
       
        {
            // First draw your main color-filled rect
            //window->DrawList->add(bb_inner.Min, bb_inner.Max, ImGui::GetColorU32(col_source), rounding);
            ImGui::RenderColorRectWithAlphaCheckerboard(
                window->DrawList,
                bb_inner.Min, bb_inner.Max,
            ...
            // Then overlay the white vertical gradient on top
            ImVec2 min = bb_inner.Min;
            ImVec2 max = bb_inner.Max;

            ImU32 white_top = ImGui::GetColorU32(ImVec4(1, 1, 1, 0.10f)); // 10% alpha white
            ImU32 white_bot = ImGui::GetColorU32(ImVec4(1, 1, 1, 0.00f)); // 0% alpha white
    ...
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
            ...
            window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_ScrollbarGrabHovered), rounding); // Color button are often in need of some sort of border
            window->DrawList->AddRect(bb.Min - ImVec2(1,1), bb.Max + ImVec2(1, 1), GetColorU32(ImGuiCol_ScrollbarGrabActive), rounding); // Color button are often in need of some sort of border
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (is_visible)
    {
        if (held || hovered)
        {
            ImColor Colr(106, 106, 108);
            if (held)
                Colr = ImColor(GetColorU32(ImGuiCol_CheckMark));
            ImGui::PushStyleColor(ImGuiCol_Text, Colr.Value);
        }
        if (selected)
        {
            ImGui::PushStyleColor(ImGuiCol_Text, ImColor(GetColorU32(ImGuiCol_CheckMark)).Value);
        }
        RenderTextClipped(pos + ImVec2(5, 0), ImVec2(ImMin(pos.x + size.x, window->WorkRect.Max.x), pos.y + size.y), label, NULL, &label_size, style.SelectableTextAlign, &bb);
        if (held || hovered)
            ImGui::PopStyleColor();
        if (selected)
            ImGui::PopStyleColor();
